...
// --- 抹平 Qt 5 和 Qt 6 鼠标坐标 API 差异的宏 ---
#if (QT_VERSION >= QT_VERSION_CHECK(6, 0, 0))
#define GET_POS(event) (event->position().toPoint())
#define GET_GLOBAL_POS(event) (event->globalPosition().toPoint())
#else
#define GET_POS(event) (event->pos())
...
void WidgetData::setMax(bool bMax)
{
    if (m_bMax != bMax)
    {
        if (bMax)
        {
            m_rect = m_pWidget->geometry();
            // 动态获取当前窗口所在的屏幕
            QScreen* screen = QGuiApplication::screenAt(m_pWidget->geometry().center());
            if (!screen) screen = QGuiApplication::primaryScreen();
            m_pWidget->setGeometry(screen->geometry());
        }
        else
        {
...
void WidgetData::handleMousePressEvent(QMouseEvent* event)
{
    if (event->button() == Qt::LeftButton)
    {
        ...
        m_bLeftButtonTitlePressed = GET_POS(event).y() < m_moveMousePos.m_nTitleHeight;

        QRect frameRect = m_pWidget->frameGeometry();
        m_pressedMousePos.recalculate(GET_GLOBAL_POS(event), frameRect);

        m_ptDragPos = GET_GLOBAL_POS(event) - frameRect.topLeft();
...
void WidgetData::handleMouseMoveEvent(QMouseEvent* event)
{
    ...
    if (m_bLeftButtonPressed)
    {
        ...
        {
            resizeWidget(GET_GLOBAL_POS(event));
        }
        else if (d->m_bWidgetMovable && m_bLeftButtonTitlePressed)
        {
            moveWidget(GET_GLOBAL_POS(event));
        }
    }
    else if (d->m_bWidgetResizable)
    {
        updateCursorShape(GET_GLOBAL_POS(event));
    }
}
...
void WidgetData::handleMouseDblClickEvent(QMouseEvent* event)
{
    if (event->button() == Qt::LeftButton)
    {
        if (GET_POS(event).y() < m_moveMousePos.m_nTitleHeight)
        {
            setMax(!m_bMax);
        }
    }
...
void WidgetData::handleHoverMoveEvent(QHoverEvent* event)
{
    ...
    {
        updateCursorShape(m_pWidget->mapToGlobal(GET_POS(event)));
    }
}
